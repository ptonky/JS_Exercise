...
function hasAZero(num) {
    return num.toString().split('').some(function(val) { //converts the numbers into a string to split into a sub-string
        return val === '0';
    })
}
...
function hasNoDuplicates(arr) {
    return arr.every(function(val) {
        return arr.indexOf(val) === arr.lastindexOf(val); //if the index of a value is equal to the last (essentially first) value (meaning no duplicates)
    });
}
